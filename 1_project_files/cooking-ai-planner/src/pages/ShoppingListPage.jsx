...
export default function ShoppingListPage() {
  ...
  const groupsWithManual = useMemo(() => {
    ...
    return hasManual ? groups : [...groups, { key: 'manual', recipe: null, items: [], total: 0, done: 0, progress: 0 }]
  }, [groups])
    ...
    <section className="page v3Page">
      <div className="v3PageHero">
        <div>
          ...
          <p className="v3PageHero__desc">按菜谱练习分组整理食材、调料和器具，做饭前先把缺的东西确认好。</p>
        </div>
      ...
      {totalItems === 0 ? (
        <div className="emptyState v3EmptyState">
          ...
          <div className="emptyState__hint">去菜谱详情页点击“加入待购清单”，或先创建一个手动清单。</div>
          ...
          {groupsWithManual.map((group) => (
            <Link key={group.key} to={`/shopping/${encodeURIComponent(group.key)}`} className="groupCard v3ListCard">
              ...
              <div className="groupCard__body">
                <div className="groupCard__title">{group.recipe ? group.recipe.title : '手动清单'}</div>
